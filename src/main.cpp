...
#if defined(NDEBUG)
# error "GothicCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x98fe093595eec9b26901a296f5a7c11de51fae97988fcaf0fd6db37187f42349");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // GothicCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "GothicCoin Signed Message:\n";
...
{
    // GothicCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 7100 * COIN;

            if(nHeight == 1)
            {
            nSubsidy = 331333333 * COIN;
            }
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // GothicCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // GothicCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1462127705, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        txNew.vout[0].nValue = 7100 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0439909b92fa1eca61c97939ea16463456e2d6f7fdc9c994c694283284e6091d4600697ec9bff98680930b4787ae5e99a8f11fea127428e7ed2c82f547ace17884") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1462127705;
        ...
        assert(block.hashMerkleRoot == uint256("0xc1578e74f5b2ff58d432717dc0b859f0e0dfb7d9bc2358669b8df77021ed6902"));
...
unsigned char pchMessageStart[4] = { 0x4a, 0x42, 0x84, 0x83 }; // GothicCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GothicCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GothicCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GothicCoinMiner(CWallet *pwallet)
{
    printf("GothicCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("gothiccoin-miner");
    ...
    try { loop {
        ...
        printf("Running GothicCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("GothicCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GothicCoinMiner, pwallet));
}
